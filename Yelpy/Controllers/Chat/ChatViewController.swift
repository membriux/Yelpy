...
class ChatViewController: UIViewController {
    ...
    @objc func retrieveChatMessages() {
        ...
        let query = PFQuery(className: "TrainingFall2021") // className = group chat name, obtained from Constants.swift
    ...
    @IBAction func onSend(_ sender: Any) {
        ...
        if messageTextField.text!.isEmpty == false {
            let chatMessage = PFObject(className: "TrainingFall2021") // className = group chat, Obtained from Constants.swift
